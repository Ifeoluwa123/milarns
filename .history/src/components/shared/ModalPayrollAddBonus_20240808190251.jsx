...
useEffect(function(){
  ...
  <div className="py-[1rem] px-[1rem] bg-[#F5F5F5] text-black rounded-[4px] ">
    ...
    <div className="">
      <InnerButton onClick={handleAddBonus} disabled={ payload.amount == '' || payload.medium == '' } bgColor={ payload.amount == '' ? "bg-grey-500 text-black cursor-not-allowed" : "bg-[var(--secondary-color)] text-white" } text={isPending? "Loading...": "Record bonus now"} width="flex justify-center w-full" />
      {/* <Button onClick={handlepayout} disabled={value.amount_left > 0 } bgColor={value.amount_left > 0 ? "bg-grey-500 text-black cursor-not-allowed" : "bg-[var(--secondary-color)] text-white" } text="Make payment now" padding="py-[0.6rem]" /> */}
    </div>
