...
useEffect(function(){
  ...
  if(staffCode){
    ...
    let handleAddBonus = (e)=>{
           ...
           mutate(payload,{
                ...
                onError: (error)=>{
                  console.log(error)
                  AlertMessages("Error Occur",error.response.data.message, "error")
                }
           })
           console.log(payload)
    }
  ...
  <div className="py-[1rem] px-[1rem] bg-[#F5F5F5] text-black rounded-[4px] ">
    ...
    <div className="">
      <InnerButton onClick={handleAddBonus} text={isPending? "Loading...": "Record bonus now"} width="flex justify-center w-full" />
