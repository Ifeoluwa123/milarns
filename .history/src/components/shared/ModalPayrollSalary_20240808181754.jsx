...
let payload = {}
  if(mutation.variables ){
    payload = {
      ...
      let {mutate, isPending} = usePayEmployeeSalary()
...

{
  ...
  <a href="#"  className="text-[var(--secondary-color)] underline">Topup wallet fund</a> 
    ...
    <div className="">
      ...
      <Button onClick={handlepayout} disabled={value.amount_left > 0  || value.total_payout == 0} bgColor={value.amount_left > 0 || value.total_payout == 0 ? "bg-grey-500 text-black cursor-not-allowed" : "bg-[var(--secondary-color)] text-white" } text={`${isPending? 'Loading...': 'Make payment now'}`} padding="py-[0.6rem]" />
    </div>
