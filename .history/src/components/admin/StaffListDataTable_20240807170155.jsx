...
export default function StaffListDataTable({staffList, itemsPerPage,addNewstaffHandler,handleExport }) {
             ...
             <tbody>
                ...
                paginatedData.map((item,i)=>{
                 ...
                 <tr    className='relative z-[10]  hover:bg-[--grey-color] rounded-md hover:shadow-[2px_2px_1px_2px_rgba(0,0,0,1)] '>
                         <td ><Checkbox id={item.id} checked={item.isChecked} onChange={()=>handleSingleCheckboxChange(item.code, data, setSelectAll,setData )} /></td>
...
{selectedUser?.code === item.code && (
  ...
  <tr  className='relative top-[-1px] bg-[var(--grey-color)]   hover:bg-[var(--grey-color)]  '>
   ...
   {/* DESKTOP VIEW */}
   <td colSpan="4"   className="mt-[-15px] hidden md:block border-2  p-0 z-[50]   rounded-md overflow-hidden absolute  md:right-[2%] md:top-[20%] bg-[var(--grey-color)]  w-fit  shadow-[2px_2px_1px_2px_rgba(0,0,0,1)]">
      ...
      {/* onClick={()=>handleActionType('delete', item.code)}  */}
