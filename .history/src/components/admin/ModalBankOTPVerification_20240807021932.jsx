...
export default function ModalBankOTPVerification({isOpens, setIsopens, msg }) {
      ...
      <ModalContent className='bg-white  rounded-md'>
        {(onClose) => (
          <>
            ...
            {/* <p className='text-gray-500 text-[1rem] md:text-[0.88rem] leading-8 '>Please input  the 6 digit character sent to {phoneNo}</p> */}
            <p className='text-gray-500 text-[1rem] md:text-[0.88rem] leading-8 '>{msg ? msg : null}</p>
            </div>

            <div className="">
            {
                errorMsg && <div className={`${errorMsg && 'bg-[#FDEAEA] text-[#FE5E55] text-[0.85rem] mb-[0rem] rounded-[4px] w-fit p-[4px_10px_4px_10px]' }`}>{errorMsg && errorMsg}</div>
               } 
            {/* {
