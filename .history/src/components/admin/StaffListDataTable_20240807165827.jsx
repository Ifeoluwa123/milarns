  ...
  <tr  className='relative top-[-1px] bg-[var(--grey-color)]   hover:bg-[var(--grey-color)]  '>
   ...
   <td colSpan="4"   className="mt-[-15px] hidden md:block  p-0 z-[50]   rounded-md overflow-hidden absolute  md:right-[2%] md:top-[20%] bg-[var(--grey-color)]  w-fit  shadow-[2px_2px_1px_2px_rgba(0,0,0,1)]">
      ...
      <TableMenuList id={item.code} items={[
        ...
        {icon:<LiaUserAltSlashSolid />, text: "Activate account", onClick:()=> handleActionType('deactivate', item.code)},      
        {icon:<LiaUserAltSlashSolid />, text: "Deactivate account", onClick:()=> handleActionType('deactivate', item.code)},      
        {icon:<CiUser/>, text: "Delete account", onClick:()=> handleActionType('delete', item.code)}      
      ]} 
      style=" px-0 w-[300]"  />
      </div>
