...
let handleDeactivateUser =  ()=>{
  ...
  mutateDeactivate(selectedUser,{
    ...
    onError:(error)=>{
      if(error){
        console.log(error.response.data.errors.codes.non_field_errors)
        if(error.response.data.errors.codes.non_field_errors){
          AlertMessages('Error',error.response.data.errors.codes.non_field_errors, 'error')
        }
        setDeactivateRecord(false)
        displayErrorMessages(error.response.data.errors.codes.non_field_errors)
      }
      // console.log(error)
    }
  })
