...
import { useActivateSingleEmployeeData, useDeactivateSingleEmployeeData, useDeleteEmployee } from '../../services/admin/mutation'
...
export default function StaffListDataTable({ addNewstaffHandler,handleExport }) {
  const [selectedUser, setSelectedUser] = useState(null);
    ...
    const [activateRecord, setActivateRecord] = useState(false)
     //For checbox
    const [bulkUserToggler, setBulkUserToggler] = useState(false)
    let itemsPerPage = 6;
...
// Single Staff Activate

let {mutate:mutateActivate, isPending:isActivatePending} = useActivateSingleEmployeeData()
let handleActivateUser =  ()=>{

  mutateActivate([selectedUser],{
    onSuccess:(success)=>{
...
// BULK STAFF RECORD CONFIGURATION

let [displayText, setDisplayText] = useState(
  {
    text:"Delete",
    icon:<LiaTrashAlt size={18} />
  }
)
...
const handleBulkSelection = (text, icon)=>{
  setBulkUserToggler(false)
  console.log(text)
  setDisplayText({
    text:text,
    icon:icon
...
const handleBulkProceed = ()=>{
  ...
  if(displayText.text == 'Deactivate'){
    return setDeactivateBulk(true) ;
  }
  if(displayText.text == 'Delete'){
    return setDeleteBulk(true);
  }
}
...
mutateDeactivate(selectedUsers,{
  ...
  onError:(error)=>{
    ...
    return (
        ...
        {/* =====>> SINGLE MODAL DISPLAY<<====== */}
         ...
         <Modals isOpens={deleteRecord} setIsopens={setDeleteRecord} title="Delete Staff">
                        <div className="">
                          ...
                          <Button onClick={handleDeleteUser} text={`${isDeletePending? 'Loading...':'Yes Proceed'}`} width="w-fit" padding="py-[0.5rem] px-[1.7rem]" />
                          <OutlineButton onClick={()=>setDeleteRecord(false)} text="Cancel"  />
                          </div>
        </Modals>
         {/* ACTIVATE MODAL */}
        <Modals isOpens={activateRecord} setIsopens={setActivateRecord} title="Activate Staff">
                 <div className="">
      ...
      {/* =====>> BULK MODAL DISPLAY<<====== */}
 {/* DELETE MODAL */}
 <Modals isOpens={deleteBulk} setIsopens={setDeleteBulk} title="Delete Staff">
                        <div className="">
                          <h6 className='font-[600] '>Are you sure you want to delete this staffs?</h6>
                            <p className='text-[0.8rem]'>Once you delete this staffs, you will not be able to retrieve it.</p>
                          </div>
         ...
         {/* ACTIVATE MODAL */}
        <Modals isOpens={activateBulk} setIsopens={setActivateBulk} title="Activate Staff">
                 <div className="">
                          <h6 className='font-[600] '>Are you sure you want to activate this staffs?</h6>
                            {/* <p className='text-[0.8rem]'>Once you deactivate this staffs, you can still reactivate staff.</p> */}
                          </div>
         ...
         {/* DEACTIVATE MODAL */}
        <Modals isOpens={deactivateBulk} setIsopens={setDeactivateBulk} title="Deactivate Staff">
                 <div className="">
                          <h6 className='font-[600] '>Are you sure you want to deactivate this staffs?</h6>
                            <p className='text-[0.8rem]'>Once you deactivate this staffs, you can still reactivate staff.</p>
                          </div>
                          ...
                          <div className="flex items-center gap-x-[2rem] mt-[1rem]">
                          <Button onClick={handleBulkDeactivateUser} text={isDeactivatePending? 'Loading..':'Yes Proceed'} width="w-fit" padding="py-[0.5rem] px-[1.7rem]" />
                          <OutlineButton onClick={()=>setDeactivateBulk(false)}  text="Cancel"  />
                          </div>
        </Modals>
     ...
     <div className="px-[0.8rem] relative sm:px-[3rem] lg:px-[2rem]   flex  flex-col gap-y-[1rem] sm:flex-row sm:items-center sm:justify-between mt-[2rem] pr-[1.8rem] " >
         ...
         <div className="flex  items-center w-full gap-x-[1rem]  sm:gap-x-[1rem]  sm:flex-row">
             <div className="rlative">
                <div className="">
                    <button onClick={()=>setBulkUserToggler(prev=>!prev)} className='flex border-[1px] rounded-[4px] py-[0.5rem] px-[0.8rem] w-fit items-center gap-x-[0.6rem] text-[0.85rem]'>
                    <span>
                      {
                         displayText.icon && displayText.icon
                      }
               ...
               <div className={` ${bulkUserToggler? 'visible opacity-1': 'invisible opacity-0 '} z-[100] transition-opacity  duration-[0.5s] absolute bottom-[110%] rounded-[4px] left-[2.7%]    bg-[var(--grey-color)] w-fit  shadow-[2px_2px_1px_2px_rgba(0,0,0,1)]`}>
               <TableMenuList  items={[
                  {icon:<CiUser size={15} />, onClick:()=>handleBulkSelection('Activate', <CiUser size={18} />), text: "Activate staffs"},     
                  {icon:<LiaUserAltSlashSolid size={15} />, onClick:()=>handleBulkSelection('Deactivate', <LiaUserAltSlashSolid size={18} />), text: "Deactivate staffs", type:"", actionType:"deactivate"},      
                  // {icon:<IoCardOutline size={15} />, text: "Activate Cards", type:"", actionType:"deactivate"},      
                  {icon:<IoTrashOutline size={15} />, onClick:()=>handleBulkSelection('Delete', <LiaTrashAlt size={18} />), text: "Delete staffs", type:"", actionType:"deactivate"},      
             ...
             <InnerButton onClick={handleBulkProceed}  text="Proceed" width="w-fit text-[0.85rem]"    />
