...
import { useAllOrgPayroll } from '../../services/admin/queries'


export default function PayRollDataTable({handleDateChange,monthYear,itemsPerPage,addNewstaffHandler }) {
  


  let {data:allStaffPayroll,  isLoading:isPayrollLoading, isError:isPayrollErr, error:payrollErr, refetch} = useAllOrgPayroll(monthYear)
  ...
  const [selectedUser, setSelectedUser] = useState(null);
    ...
    let [data,setData] = useState(allStaffPayroll?.results)

    useEffect(()=>{
      const updatedItems = allStaffPayroll.results.map(item => ({ ...item, checked: false }));
      setData(updatedItems)
    },[allStaffPayroll?.results])
