  ...
  let handleOnSubmit = (e) => {
    ...
    if (validateUserData(userData, setErrMsg)) {
      return mutate(userData, {
        ...
        onError: (err) => {
          if (
            err.response.data.message.includes("Account verification required")
          ) {
            return navigate("/verify-otp-reg");
          }
        ...
        <div className="my-[1.4rem]">
          {errMsg.error && (
            <div
              className={`${
                errMsg.error &&
                "bg-[#FDEAEA] text-[#FE5E55] mb-[1rem] rounded-[4px] w-fit p-[4px_20px_4px_20px]"
              }`}
            >
              {errMsg.error}
            </div>
          ...
          {errMsg.success && (
            <div
              className={`${
                errMsg.success &&
                "bg-[var(--primary-color)] text-white mb-[1rem] rounded-[4px] w-fit p-[4px_20px_4px_20px]"
              }`}
            >
              {errMsg.success}
            </div>
