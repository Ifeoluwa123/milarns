  ...
  let handleOnSubmit = (e) => {
    ...
    if (validateUserData(userData, setErrMsg)) {
      return mutate(userData, {
        onSuccess: (result) => {
          sessionStorage.setItem("phone", userData.phone_number)
    ...
    <div>
      <form onSubmit={handleOnSubmit}>
        ...
        <div className='pt-[3rem] pb-[3rem] '>
          <p className='text-[1rem]'> Have an account with WagePull? <Link to="/" className='text-[var(--secondary-color)]'>Login here</Link></p>
        </div>
      </form>
    </div>
  )
