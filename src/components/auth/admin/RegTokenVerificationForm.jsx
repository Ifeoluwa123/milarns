...
export default function RegTokenVerificationForm({ otpSentMsg, setOTPSentMsg, sendIsSentPhone }) {
  ...
  const handleResendCode = (e) => {
    ...
    e.preventDefault()
    let payload = ''
    if (isSentPhone) { payload = sessionStorage.getItem('phone') }
    else { payload = sessionStorage.getItem('email') }
  ...
  const handleGetCodeBySmsEmail = (e) => {
    setIsSentPhone(!isSentPhone)
    sendIsSentPhone(isSentPhone);
  }
  ...
  return (
    <>
      ...
      <div onClick={handleGetCodeBySmsEmail} className="text-center my-[1rem] underline cursor-pointer">
        {isSentPhone ? <p>Get Code by SMS</p> : <p>Get Code by Email</p>}
