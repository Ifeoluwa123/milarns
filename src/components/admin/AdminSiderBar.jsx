...
function MobileView({ pathname, isSidebarOpen, setIsSideBarOpen }) {
  ...
  return (
    <div
      ...
      } transition-all duration-[0.3s]   flex justify-between bg-[rgba(0,0,0,0.7)] h-screen md:hidden fixed top-0 w-full z-[200]`}
    >
        ...
        <div className="text-white border- border-red-900 h-fit absolute top-[5rem] md:top-[7rem] pl-[1.5rem] w-full ">
          <div className="">
            ...
            <h4 className="mt-[0.5rem]">
              {adminInfo.organization.director_name}{" "}
            </h4>
              ...
              return (
                <li key={index} onClick={() => setIsSideBarOpen(false)}>
                  <Link
                    ...
                    } transition-all duration-[0.3s] ease py-[0.6rem] px-[0.6rem] mb-[1rem] rounded-md  flex hover:bg-[var(--secondary-color)] items-center gap-[1rem]`}
                  >
                    <span>{item.icon}</span>
                    {item.name}
                  </Link>
      ...
      <div className={`pt-[0.6rem] pr-[0.6rem]`}>
        <div
          className="text-black bg-white hover:cursor-pointer ml-[2rem]  rounded-full"
...
function DesktopView({ pathname, isSidebarOpen, setIsSideBarOpen }) {
  ...
  let { data } = useOrgSettingsInfo();

  return (
    <div
      className={` bg-white border-r-[1px] border-[#F5F5F5] h-screen fixed ${
        isSidebarOpen ? "max-w-[200px]" : "max-w-[85px]"
      } transition-all duration-[0.3s] z-[100]  w-full hidden md:block`}
    >
      <div className=" py-[0.88rem] border-b-[1px]">
        <Link
          to="admin"
          className={`${
            !isSidebarOpen ? "max-w-[25px] w-full" : "w-[75px] "
          }    mx-auto block`}
        >
          <img
            src={!isSidebarOpen ? Logo2 : Logo}
            className={` w-full object-cover block `}
            alt=""
          />
        </Link>
      ...
      <div
        className={` ${
          !isSidebarOpen && "bg-[var(--primary-color)]"
        }  transition-all duration-[0.3s] absolute pt-[3rem] flex items-center flex-col justify-center w-full  `}
      >
        <div
          className={`${
            isSidebarOpen ? "w-[80px]   " : "w-[47.32px]"
          } transition-all duration-[0.3s] rounded-full  overflow-hidden`}
        >
          <img
            src={data?.logo ? data?.logo : userPlaceholder}
        ...
        <div
          className={`${
            isSidebarOpen ? "opacity-1 mb-[3.2rem]" : "opacity-0"
          } whitespace-nowrap text-white transition-all duration-[0.3s]`}
        >
          <h4 className="mt-[0.5rem]">
            {adminInfo.organization.director_name}
          </h4>
      ...
      <div className="text-white mt-[0rem]  md:mt-[5.7rem] bg-[var(--primary-color)] h-full ">
        <ul
          className={`relative ${
            !isSidebarOpen ? "top-[2rem]" : "top-[8rem]"
          } transition-all duration-[0.3s] h-full   overflow-y-auto`}
        >
          {AdminLinks.map((link, index) => (
            ...
            <li key={index} className="mb-[1rem] ml-[1rem]">
              ...
              <NavLink
                ...
                className={`text-white whitespace-nowrap pl-[0.5rem] rounded-tl-md rounded-bl-md ${
                  pathname.includes(link.path) && "bg-[var(--secondary-color)]"
                } transition-all duration-[0.3s] ease py-[0.6rem]   flex hover:bg-[var(--secondary-color)] items-center gap-[1rem]`}
              >
