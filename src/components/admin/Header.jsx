...
export default function Header({isSidebarOpen, setIsSideBarOpen}) {
  ...
  return (
    <header
      className={`fixed top-0 bg-[var(--grey-color)]    ${
        isSidebarOpen
          ? "md:max-w-[calc(100%-200px)] md:left-[200px]"
          : "md:max-w-[calc(100%-85px)] md:left-[85px]"
      } transition-all duration-[0.3s]   w-full border-b-[1px] py-[1rem] md:py-0 m-0 z-[120] bg-white md:bg-transparent md:z-[50] px-[1rem] md:px-0`}
    >
      <div className="flex justify-between">
        ...
        <div className="hamburger flex items-center gap-x-[0.6rem]  md:max-w-[250px]  lg:max-w-[300px] overflow-x-hidden md:pl-[1.5rem] w-full md:h-[51px] md:bg-[var(--grey-color)]">
          <span
            ...
            className="text-[1rem] cursor-pointer md:hidde"
          >
            <FaBarsStaggered />
          </span>
          {countSlashes > 1 && (
            <span
              onClick={() => navigate(pathname.split("/").splice(1)[0])}
          ...
          <h3 className="text-[1rem] font-[700] whitespace-nowrap ">
            {urlPath}
          </h3>
        </div>
        {/* Right Box*/}
        <div className="md:mr-[1rem] lg:mr-[3rem]">
          <div className="flex items-center h-full ">
            ...
            <a
              href="https://milarn.com/our-support/"
              target="_blank"
              className="flex items-center   font-[500] whitespace-nowrap "
            >
              <span className="text-[1.3rem]">
                <IoIosHelpCircleOutline />
              </span>{" "}
              &nbsp; <span className="  text-[0.85rem]">Support</span>
            </a>
            <div
              onClick={() => setIsLoginBar((prev) => !prev)}
              className=" cursor-pointer flex items-center gap-[0.5rem] ml-[0.8rem] md:ml-[2rem]"
            >
              ...
              <div className="   ">
                ...
                <div
                  className={` ${
                    isLoginBar ? "visible" : "invisible"
                  }  absolute top-[100%] right-[-2%]  md:right-[-5%] lg bg-[var(--grey-color)] w-[300px] max-w-[150px] mr-[1rem] md:mr-[5rem] shadow-[2px_2px_1px_1px_rgba(0,0,0,1)] rounded-sm `}
                >
                  <ul className="relative md:text-black">
                    <li className="border-b-[0.3px]">
                      <Link
                        to="/settings"
                        className="text-[0.85rem]  flex items-center justify-center  gap-[1rem] py-[1rem] hover:bg-[--secondary-color-hover-lighter] hover:text-[var(--secondary-color)]"
                      >
                        <span>
                          <CiUser />
                        </span>{" "}
                    ...
                    <li onClick={logout}>
                      <Link className="text-[0.85rem]  flex items-center justify-center  gap-[1rem] py-[1rem] hover:bg-[--secondary-color-hover-lighter] hover:text-[var(--secondary-color)]">
                        <span>
                          <IoLogOutOutline />
                        </span>{" "}
                        Log out{" "}
