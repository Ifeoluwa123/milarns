    ...
    <AdminCustomLayout>
      ...
      <ComponentLeft>
        <OrgnanizationAdminCard
          ...
          status={true}
          bankName={
            data?.bankinformation.bank_name
              ? data.bankinformation.bank_name
              : "Nil"
          }
          accNumber={
            data?.bankinformation.account_number
              ? data?.bankinformation.account_number
              : "Nil"
          }
          accName={
            data?.bankinformation.account_name
              ? data?.bankinformation.account_name
              : "Nil"
          }
        />
      ...
      <ComponentRight>
        ...
        <div className=" flex gap-x-[3rem] border-b-[0.5px]  overflow-x-auto no-scrollbar">
          {tabButton.map(({ label, activeIndex }) => (
            <Tab
              key={label}
              label={label}
              activeStyle={` ${
                activeIndex == activeTab &&
                "text-[--secondary-color] border-b-[1px] border-[--secondary-color]"
              }`}
        ...
        <div className="mt-[2rem]">
          ...
          {/* {activeTab === 1 && (
            <AdminSalaryAccount data={data.bankinformation} />
          )} */}
